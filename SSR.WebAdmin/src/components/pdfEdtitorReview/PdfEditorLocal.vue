<script>
import {readAsImage, readAsPDF, readAsDataURL} from '../pdfEditor/utils/asyncReader'
import prepareAssets, {fetchFont} from '../pdfEditor/utils/prepareAssets'
import {save} from '../pdfEditor/utils/PDF'
import {generateId} from '../pdfEditor/utils/helper'
...
export default {
  ...
  components: {Countdown, ObjectContainer, PdfPage, LetterCube, Switches},
  props: {
    pdf: {
      required: false,
      type: String,
  ...
  methods: {
    async handleAssignOrReject() {
      this.submitted = true;
      this.$v.$touch();
      console.log(this.$v.signatureDigital.$invalid)
      if (this.$v.signatureDigital.$invalid) {
        return;
      }
      ...
      this.signatureDigital.vanBanDiId = this.$route.params.id;
      await this.$store.dispatch("signDigitalStore/kySoNoiBoV2", this.signatureDigital).then((res) => {
        if (res.resultCode == 'SUCCESS') {
          this.resultData.resultCode = res.resultCode;
          this.resultData.resultString = res.resultString;
        ...
        if (res.resultCode == "SUCCESS") {
          if (res.data) {
            ...
            if (this.model.signDigitalLocals) {
              this.allObjects = this.model.signDigitalLocals;
    ...
    async handleSubmit(e) {
      ...
      this.signatureDigital.userName = this.currentUser.userNameKySo;
      this.signatureDigital.password = this.currentUser.passwordKySo;
      this.signatureDigital.vanBanDiId = this.$route.params.id;
      await this.$store.dispatch("signDigitalStore/kySoNoiBoV2", this.signatureDigital).then((res) => {
        if (res.resultCode == 'SUCCESS') {
          this.resultData.resultCode = res.resultCode;
...
<template>
  <div class="tw-mb-4 tw-flex tw-flex-col tw-relative kyso-box">
    ...
    <div class="footer-kyso d-flex justify-content-end">
      ...
      <button
          ...
          @click="showModelAcceptKySo = true"
          style="background-color: rgba(12, 92, 168, 0.1)"
    ...
    >
      <template #modal-header="{  }">
        ...
        <div class="text-end">
          ...
          <b-button v-if="!signatureDigital.reject" v-b-modal.modal-close_visit
                    size="sm"
                    variant="primary"
                    type="button"
                    class="w-md "
      ...
      <div class="row" ref="modalAcceptKySo">
        <div class="col-md-12">
          <div class=" d-flex align-items-center">
            <switches v-model="signatureDigital.reject" color="primary" class="ml-1 mx-2"></switches>
            <label v-if="signatureDigital.reject" for=""> Từ chối ký </label>
        ...
        <div class="col-md-12 mt-2">
          ...
          <input
              ...
              v-model="signatureDigital.password"
              type="password"
              class="form-control"
              :class="{
                                'is-invalid':
                                  submitted && $v.signatureDigital.password.$error,
                                }"
        />
          <div
              v-if="submitted && !$v.signatureDigital.password.required"
              class="invalid-feedback"
          >
        ...
        <div class="col-md-12 mt-2">
          ...
          <textarea
              id="validationCustom01"
              v-model="signatureDigital.content"
              type="text"
              class="form-control"
              :class="{
                                'is-invalid':
                                  submitted && $v.signatureDigital.content.$error,
                                }"
          ></textarea>
          <div
              v-if="submitted && !$v.signatureDigital.content.required"
              class="invalid-feedback"
          >
            Nội dung không được để trống.
          </div>
        ...
        <div class="row justify-content-center">
          <div class="col-md-12"
            ...
            <button type="button" v-on:click="handleHideModal" class="btn btn-modal text-uppercase"
                    style="width: 200px; margin-top: 20px; border-radius: 30px">Trở về
            </button>
          </div>
        </div>
      </Transition>
