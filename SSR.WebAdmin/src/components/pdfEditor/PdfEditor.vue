...
export default {
  ...
  components: {Countdown, ObjectContainer, PdfPage, LetterCube, Multiselect},
  props: {
    pdf: {
      required: false,
      type: String,
  ...
  methods: {
    async handleResetNhomNguoiLanhDao() {
      let loader = this.$loading.show({
        container: this.$refs.formContainer,
      });
      await this.$store.dispatch("vanBanDiStore/resetNhomNguoiTiepNhan", this.$route.params.id).then((res) => {
    ...
    initLanhDao(){
      if(this.model && this.model.nhomNguoiTiepNhanVBTrinhLD){
        var userLD = this.model.nhomNguoiTiepNhanVBTrinhLD.find(x => x.roleCode == "9999");
        if(userLD && userLD.nguoiXuLy){
          var tempLD = this.optionsUsersLanhDao.find(x => x.userName == userLD.nguoiXuLy.userName);
          this.signatureDigital.userLanhDao = tempLD;
        }
        this.checkNhomNguoiTiepNhan = false;
      }else{
        this.checkNhomNguoiTiepNhan = true;
      }
    },
    async getUsersLanhDao() {
      try {
        await this.$store.dispatch("userStore/getUsersLanhDao").then(resp => {
          if (resp.resultCode == "SUCCESS") {
            let items = resp.data
            this.optionsUsersLanhDao = items;
          }
          return [];
        });
      } finally {
    ...
    async handleSubmitKySoPhapLy(e) {
      ...
      await this.$store.dispatch("signDigitalStore/thucHienKySoPhapLy", this.signatureDigital).then((res) => {
        if (res.resultCode == 'SUCCESS') {
          ...
          this.fileName = res.data.responseContent;
    ...
    downloadPDF() {
      ...
      const fileName = this.fileName?this.fileName: moment().format("dMMyyyyhhmmss") + ".pdf";
        ...
        this.$refs.cropper.getCropData((data) => {
          try {
            ...
            if (this.signatureVM && this.signatureVM.signatureSaves) {
              ...
              if (object.typeKySo == "SIGN" && dsKySo.length >= 5) {
                dsKySo.shift()
              } else if (object.typeKySo == "COMMENT" && dsKyNhay >= 5) {
                dsKyNhay.shift();
              }
        ...
        <div class="d-flex row nav-kyso-box">
          ...
          <div class="nav-kyso-right d-flex flex-wrap col-lg-6 col-md-6 col-sm-12">
            <button
                ...
                @click="handleResetNhomNguoiLanhDao"
            >
            ...
            <div class="btn-group ">
              ...
              >
                ...
                <div style="padding: 0 20px; max-height: 400px; overflow-y: scroll">
...
<!--                    </button>-->
      <button
                        v-if="model.trangThai && model.trangThai.code == 'tkhtxn' && model.ower && model.ower.userName == currentUserName"
                        class="btn text-primary px-4 fw-semibold me-2"
                        @click="isShowModalChonLanhDao = true"
                        style="background-color: rgba(12, 92, 168, 0.1)"
                        :disabled="!checkObjectExited"
                    >
                      Chấp nhận thiết lập
...
<!--    Modal chọn lãnh đạo ký số -->
    <b-modal v-model="isShowModalChonLanhDao" centered title="Thiết lập lãnh đạo ký số" title-class="font-18" no-close-on-backdrop
             size="md" ref="formContainerTrangThai">
      <div class="row">
        <div class="col-md-12">
          <div class="mb-2">
            <label class="form-label" for="validationCustom01">Chọn lãnh đạo</label>
            <multiselect v-model="signatureDigital.userLanhDao" :options="optionsUsersLanhDao" track-by="id" label="fullName"
                         placeholder="Chọn lãnh đạo" deselect-label="Nhấn để xoá" selectLabel="Nhấn enter để chọn"
      ...
      <template #modal-footer>
        <b-button v-b-modal.modal-close_visit size="sm" class="btn btn-outline-info w-md"
                  v-on:click="isShowModalChonLanhDao = false">
          Đóng
        </b-button>
        <b-button
                  v-b-modal.modal-close_visit size="sm" variant="primary" type="button" class="w-md"
                  :disabled="signatureDigital.userLanhDao == null" v-on:click="handleSubmit">
          Thiết lập ký số
        </b-button>
      </template>
    </b-modal>
