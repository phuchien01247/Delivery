...
export default {
  ...
  watch: {
    showModal(status) {
      if (status == false) this.model = categoryModel.baseJson();
    },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
  ...
  methods: {
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("categoryStore/getById", id).then((res) => {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("categoryStore/delete", this.model.id).then((res) => {
          if (res.resultCode === 'SUCCESS') {
            this.fnGetList();
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("categoryStore/update", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
          ...
          // Create model
          await this.$store.dispatch("categoryStore/create", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
              this.showModal = false;
              this.model = categoryModel.baseJson();
            }
            this.$store.dispatch("snackBarStore/addNotify", notifyModel.addMessage(res))
          });
        }
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("categoryStore/getPagingParams", params)
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-4 col-12 d-flex align-items-center">
                <h4 class="font-size-18 fw-bold cs-title-page">Loại danh mục</h4>
              </div>
              <div class="col-md-8 col-12 text-end">
                ...
                <b-button
                    ...
                    size="sm"
                >
                  <i class="mdi mdi-plus me-1"></i> Tạo loại danh mục
                </b-button>
              </div>
            </div>
                  ...
                  >
                    <form @submit.prevent="handleSubmit"
                      ...
                      <div class="row">
                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Mã loại danh mục</label>
                            ...
                            >
                              Mã loại danh mục không được để trống.
                            </div>
                          </div>
                        </div>
                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Tên loại danh mục</label>
                            <span style="color: red">&nbsp;*</span>
                        ...
                        <div class="col-6">
                          <div class="mb-3">
                            ...
                            <input
                                ...
                                :class="{
                                'is-invalid':
                                  submitted && $v.model.level.$error,
                              }"
                            />
                            <div
                                v-if="submitted && !$v.model.level.required"
                                class="invalid-feedback"
                            >
                        ...
                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Thumbnail</label>
                            <span style="color: red">&nbsp;*</span>
                            <input type="hidden" v-model="model.id"/>
                            <input
                                id="thumbnail"
                                v-model.trim="model.thumbnail"
                                type="text"
                                class="form-control"
                                placeholder="Nhập thumbnail"
                                :class="{
                                'is-invalid':
                                  submitted && $v.model.thumbnail.$error,
                              }"
                            />
                            <div
                                v-if="submitted && !$v.model.thumbnail.required"
                                class="invalid-feedback"
                            >
                              Thumbnail không được để trống.
                            </div>
                          </div>
                        </div>
                        <div class="col-6">
                          <div class="mb-3">
                            ...
                            <input
                                id="icon"
                                v-model.trim="model.icon"
                                type="text"
                                class="form-control"
                                placeholder="Nhập icon"
                                :class="{
                                'is-invalid':
                                  submitted && $v.model.icon.$error,
                              }"
                            />
                            <div
                                v-if="submitted && !$v.model.icon.required"
                                class="invalid-feedback"
                            >
                        ...
                        <div class="col-12">
                          <div class="mb-3">
                            <label class="text-left">Tóm tắt</label>
                            <span style="color: red">&nbsp;*</span>
                            <input type="hidden" v-model="model.id"/>
                            <textarea
                                id="summary"
                                ...
                                :class="{
                                'is-invalid':
                                  submitted && $v.model.summary.$error,
                              }"
                            >
                            </textarea>
                            <div
