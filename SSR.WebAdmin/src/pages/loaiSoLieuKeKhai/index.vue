...
import {loaiSoLieuKeKhaiModel} from "@/models/loaiSoLieuKeKhaiModel";

export default {
  page: {
    title: "Loại số liệu kê khai",
  ...
  data() {
    return {
      ...
      submitted: false,
      model: loaiSoLieuKeKhaiModel.baseJson(),
  ...
  watch: {
    showModal(status) {
      if (status == false) this.model = loaiSoLieuKeKhaiModel.baseJson();
    },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
  ...
  methods: {
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("loaiSoLieuKeKhaiStore/getById", id).then((res) => {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("loaiSoLieuKeKhaiStore/delete", this.model.id).then((res) => {
          if (res.resultCode === 'SUCCESS') {
            this.fnGetList();
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("loaiSoLieuKeKhaiStore/update", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
          ...
          // Create model
          await this.$store.dispatch("loaiSoLieuKeKhaiStore/create", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
              this.showModal = false;
              this.model = loaiSoLieuKeKhaiModel.baseJson();
            }
            this.$store.dispatch("snackBarStore/addNotify", notifyModel.addMessage(res))
          });
        }
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("loaiSoLieuKeKhaiStore/getPagingParams", params)
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-4 col-12 d-flex align-items-center">
                <h4 class="font-size-18 fw-bold cs-title-page">Loại số liệu kê khai</h4>
              </div>
            ...
            <div class="row mb-2">
              <div class="col-sm-8">
                <div class="text-sm-end">
                  <b-modal
                      v-model="showModal"
                      title="Thông tin số liệu kê khai"
                  ...
                  >
                    <form @submit.prevent="handleSubmit"
                      ...
                      <div class="row">
                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Mã số liệu kê khai</label>
                            ...
                            <input
                                ...
                                type="text"
                                class="form-control"
                                placeholder="Nhập mã số liệu kê khai"
                                :class="{
                                'is-invalid':
                            ...
                            >
                              Mã số liệu kê khai không được để trống.
                            </div>
                          </div>
                        </div>
                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Tên loại kê khai</label>
                            <span style="color: red">&nbsp;*</span>
                            ...
                            <input
                                ...
                                type="text"
                                class="form-control"
                                placeholder="Nhập tên loại kê khai"
                                :class="{
                                'is-invalid':
                            ...
                            <div
                                ...
                                class="invalid-feedback"
                            >
                              Tên loại kê khai không được để trống.
                            </div>
                          </div>
                        </div>
                        <div class="col-6">
                        ...
                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Số liệu kê khai</label>
                            <span style="color: red">&nbsp;*</span>
                            <input
                                id="ten"
                                v-model.trim="model.soLieuKeKhai"
                                type="text"
                                class="form-control"
                                placeholder="Nhập số liệu kê khai"
                                :class="{
                                'is-invalid':
                                  submitted && $v.model.soLieuKeKhai.$error,
                              }"
                            />
                            <div
                                v-if="submitted && !$v.model.soLieuKeKhai.required"
                                class="invalid-feedback"
                            >
                              Số liệu kê khai không được để trống.
                            </div>
                          </div>
                        </div>
                      </div>
