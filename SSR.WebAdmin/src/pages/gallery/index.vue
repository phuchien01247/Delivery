...
export default {
  ...
  watch: {
    showModal(status) {
      if (status == false) this.model = galleryModel.baseJson();
    },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
  ...
  methods: {
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("galleryStore/getById", id).then((res) => {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("galleryStore/delete", this.model.id).then((res) => {
          if (res.resultCode === 'SUCCESS') {
            this.fnGetList();
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("galleryStore/update", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
          ...
          // Create model
          await this.$store.dispatch("galleryStore/create", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
              this.showModal = false;
              this.model = galleryModel.baseJson();
            }
            this.$store.dispatch("snackBarStore/addNotify", notifyModel.addMessage(res))
          });
        }
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("galleryStore/getPagingParams", params)
    ...
    addHinhAnh(file, response){
      if(this.model){
        ...
        this.model.name = fileSuccess.fileName;
      }
    },
    clearSearch(){
      this.filter = null;
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-4 col-12 d-flex align-items-center">
                <h4 class="font-size-18 fw-bold text-dark">Hình ảnh</h4>
              </div>
              <div class="col-md-8 col-12 text-end">
                ...
                <b-button
                    ...
                    size="sm"
                >
                  <i class="mdi mdi-plus me-1"></i> Thêm hình ảnh
                </b-button>
              </div>
            </div>
