...
import { mauBaoCaoModel } from "../../models/mauBaoCao";
...
export default {
  ...
  watch: {
    showModal(status) {
      if (status == false) this.model = mauBaoCaoModel.baseJson();
    },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
  ...
  methods: {
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("mauBaoCaoStore/getById", id).then((res) => {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("mauBaoCaoStore/delete", this.model.id).then((res) => {
          if (res.resultCode === 'SUCCESS') {
            this.fnGetList();
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("mauBaoCaoStore/update", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
          ...
          // Create model
          await this.$store.dispatch("mauBaoCaoStore/create", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
              this.showModal = false;
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("mauBaoCaoStore/getPagingParams", params)
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-4 col-12 d-flex align-items-center">
                <h4 class="font-size-18 fw-bold cs-title-page"> {{ title }}</h4>
              </div>
                      ...
                      <div class="row">
                        <div class="col-12">
                          <div class="mb-3">
                            ...
                            >
                              Tên mẫu báo cáo không được để trống.
                            </div>
                          </div>
                        </div>
                        <div class="col-6">
                        ...
                        <div class="col-6">
                          <div class="mb-3">
                            <label class="text-left">Sử dụng</label>
                            <br />
                            <switches class="mt-2 mb-0"  v-model="model.dangSuDung" type-bold="false" color="success" ></switches>
                          </div>
                        </div>
                      </div>
              ...
              <div class="col-12">
                ...
                <div class="table-responsive-sm">
                  ...
                  >
                    ...
                    <template v-slot:cell(dangSuDung)="data">
                      <span v-if="data.item.dangSuDung" class="badge bg-success ms-1">Sử dụng</span>
                      <span v-else class="badge bg-danger ms-1">Ẩn</span>
                    </template>
