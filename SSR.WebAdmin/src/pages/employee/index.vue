...
export default {
  ...
  components: {Layout, Switches, vueDropzone: vue2Dropzone, Multiselect},
  data() {
    return {
      title: "Nhân viên",
      data: [],
      ...
      fields: [
        ...
        },
        {
          ...
          label: "Chức vụ",
          sortable: true,
          thStyle: {width: '140px', minWidth: '80px'}
        },
        {
          key: "isShow",
          label: "Hiển thị",
  ...
  watch: {
    showModal(status) {
      if (status == false) this.model = employeeModel.baseJson();
    },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
  ...
  methods: {
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("employeeStore/getById", id).then((res) => {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("employeeStore/delete", this.model.id).then((res) => {
          if (res.resultCode === 'SUCCESS') {
            this.fnGetList();
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("employeeStore/update", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
          ...
          // Create model
          await this.$store.dispatch("employeeStore/create", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
              this.showModal = false;
              this.model = employeeModel.baseJson();
            }
            this.$store.dispatch("snackBarStore/addNotify", notifyModel.addMessage(res))
          });
        }
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("employeeStore/getPagingParams", params)
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-4 col-12 d-flex align-items-center">
                <h4 class="font-size-18 fw-bold text-dark">Nhân viên</h4>
              </div>
              <div class="col-md-8 col-12 text-end">
                ...
                <b-button
                    ...
                    size="sm"
                >
                  <i class="mdi mdi-plus me-1"></i> Tạo nhân viên
                </b-button>
              </div>
            </div>
                  ...
                  >
                    <form @submit.prevent="handleSubmit"
                      ...
                      <div class="row">
                        <div class="col-12">
                          <div class="mb-3">
                            <label class="text-left">Tên nhân viên</label>
                ...
                <div class="table-responsive-sm">
                  ...
                  >
                    ...
                    <template v-slot:cell(chucVu)="data">
                     <template v-if="data.item.chucVu">
