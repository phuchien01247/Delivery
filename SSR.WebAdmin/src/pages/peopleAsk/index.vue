...
import {peopleAskModel} from "@/models/peopleAskModel";

export default {
  page: {
    title: "Cảm nghĩ người sử dụng",
  ...
  watch: {
    showModal(status) {
      if (status == false) this.model = peopleAskModel.baseJson();
    },
    showDeleteModal(val) {
      if (val == false) {
        this.model.id = null;
  ...
  methods: {
    ...
    async handleUpdate(id) {
      await this.$store.dispatch("peopleAskStore/getById", id).then((res) => {
    ...
    async handleDelete() {
      if (this.model.id != 0 && this.model.id != null && this.model.id) {
        await this.$store.dispatch("peopleAskStore/delete", this.model.id).then((res) => {
          if (res.resultCode === 'SUCCESS') {
            this.fnGetList();
    ...
    async handleSubmit(e) {
      ...
      if (this.$v.$invalid) {
        ...
        if (
          ...
          // Update model
          await this.$store.dispatch("peopleAskStore/update", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
          ...
          // Create model
          await this.$store.dispatch("peopleAskStore/create", this.model).then((res) => {
            if (res.resultCode === 'SUCCESS') {
              this.fnGetList();
              this.showModal = false;
              this.model = peopleAskModel.baseJson();
            }
            this.$store.dispatch("snackBarStore/addNotify", notifyModel.addMessage(res))
          });
        }
    ...
    myProvider (ctx) {
      ...
      this.loading = true
      try {
        let promise =  this.$store.dispatch("peopleAskStore/getPagingParams", params)
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-md-4 col-12 d-flex align-items-center">
                <h4 class="font-size-18 fw-bold text-dark"> Người sử dụng</h4>
              </div>
              <div class="col-md-8 col-12 text-end">
                ...
                <b-button
                    ...
                    size="sm"
                >
                  <i class="mdi mdi-plus me-1"></i> Tạo cảm nghĩ
                </b-button>
              </div>
            </div>
                  ...
                  >
                    <form @submit.prevent="handleSubmit"
                      ...
                      <div class="row">
                        <div class="col-12">
                          <div class="mb-3">
                            <label class="text-left">Tên người </label>
                        ...
                        <div class="col-12">
                          <div class="mb-3">
                            <label class="text-left"> Gắn thẻ </label>
                            <span style="color: red">&nbsp;*</span>
                            <input
                                id="ten"
                                v-model.trim="model.tag"
                                type="text"
                                class="form-control"
                                placeholder=""
                                :class="{
                                'is-invalid':
                                  submitted && $v.model.tag.$error,
                              }"
                            />
                            <div
                                v-if="submitted && !$v.model.tag.required"
                                class="invalid-feedback"
                            >
